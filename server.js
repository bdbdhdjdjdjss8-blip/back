...
// Trust proxy so req.ip works correctly behind Render
app.set('trust proxy', true);

app.post('/log', (req, res) => {
    const timestamp = req.body.timestamp;
    const url = req.body.url;
    const ip = req.ip; // visitor IP
    const userAgent = req.headers['user-agent']; // device/browser info
